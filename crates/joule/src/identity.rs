...
/// Cached pool API key from identity file (issued on agent Welcome).
pub fn cached_api_key(path: &Path) -> Option<String> {
    load(path).ok().and_then(|id| id.api_key)
}

/// Default OpenAI-compatible base (…/v1) for Cursor / Continue / etc.
pub fn openai_base_url(http_api: &str) -> String {
    let base = http_api.trim().trim_end_matches('/');
    if base.is_empty() {
        return "http://127.0.0.1:7700/v1".into();
    }
...
/// Path for dumb-easy CONNECT card (next to identity.json).
pub fn connect_note_path(identity_path: &Path) -> PathBuf {
    identity_path
        .parent()
        .map(|p| p.join("JOULE-CONNECT.txt"))
        .unwrap_or_else(|| PathBuf::from("JOULE-CONNECT.txt"))
}

/// Write JOULE-CONNECT.txt — Base URL + full API key + model for external apps.
pub fn write_connect_note(
    identity_path: &Path,
    http_api: &str,
    api_key: &str,
    model: &str,
) -> Result<PathBuf> {
    let path = connect_note_path(identity_path);
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
    let base = openai_base_url(http_api);
    let body = format!(
        r#"================================================================================
  JOULE → CONNECT OTHER APPS (Cursor, Continue, VS Code, etc.)
================================================================================

Paste these THREE fields into any OpenAI-compatible client.
You do NOT invent them. joule already made the API key for you.

--------------------------------------------------------------------------------
1) BASE URL  (sometimes called API base / OpenAI base / endpoint)
--------------------------------------------------------------------------------
{base}

--------------------------------------------------------------------------------
2) API KEY  (secret — like a password for THIS pool account)
--------------------------------------------------------------------------------
{api_key}
...
--------------------------------------------------------------------------------
HOW TO PASTE (Cursor example)
--------------------------------------------------------------------------------
1. Open Cursor Settings → Models / OpenAI-compatible / API
2. Base URL  =  the BASE URL above
3. API Key   =  the API KEY above
4. Model     =  the MODEL above
5. Save. Chat works when your joule agent is running and earning millijoules.

Other apps (Continue, Aider, Open WebUI, …): same three fields.

Show / copy again anytime:
  joule connect
  joule connect --copy          # copies API key to clipboard
  joule connect --copy-url      # copies Base URL
  joule connect --open          # open this file

Agent must have joined the pool once so a key exists. If API KEY is missing,
run:  joule agent
then: joule connect

================================================================================
Generated by joule · keep private (API key spends your millijoules)
================================================================================
"#
    );
...
/// Idiot-proof on-screen CONNECT card. Writes/refreshes JOULE-CONNECT.txt when key present.
pub fn print_connect_card(
    identity_path: &Path,
    http_api: &str,
    api_key: Option<&str>,
    model: &str,
    ...
    let owned = api_key
        ...
        .filter(|k| !k.is_empty())
        .map(|s| s.to_string())
        .or_else(|| cached_api_key(identity_path));

    println!();
    println!("======================================================================");
    println!("  CONNECT OTHER APPS (Cursor / Continue / any OpenAI client)");
    println!("======================================================================");
    println!();
    println!("  Copy these into the app. Do not invent them.");
    println!();
    println!("  Base URL:  {base}");
    match owned.as_deref() {
        Some(k) => {
            println!("  API key:   {k}");
            if let Ok(p) = write_connect_note(identity_path, http_api, k, model) {
                println!();
                println!("  Saved to:  {}", p.display());
                println!("             (open that file if you hate terminals)");
            }
        }
        None => {
            println!("  API key:   (not yet — start the agent once so the pool issues one)");
            println!("             run:  joule agent");
            println!("             then: joule connect");
        }
    }
    println!("  Model:     {model}");
    println!();
    println!("  Copy key:  joule connect --copy");
    println!("  Copy URL:  joule connect --copy-url");
    println!("  Open file: joule connect --open");
    println!("======================================================================");
    println!();
}
...
mod tests {
    ...
    #[test]
    fn openai_base_url_appends_v1() {
        assert_eq!(
            openai_base_url("http://127.0.0.1:7700"),
            "http://127.0.0.1:7700/v1"
        );
        assert_eq!(
            openai_base_url("http://127.0.0.1:7700/v1"),
            "http://127.0.0.1:7700/v1"
        );
    }

    #[test]
    fn connect_note_contains_key_url_model() {
        let n = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("joule-connect-{n}"));
        fs::create_dir_all(&dir).unwrap();
        let id_path = dir.join("identity.json");
        let id = Identity::generate();
        save(&id_path, &id).unwrap();
        remember_api_key(&id_path, "joule_deadbeefcafebabe").unwrap();
        let note = write_connect_note(
            &id_path,
            "http://127.0.0.1:7700",
            "joule_deadbeefcafebabe",
            "kimi-open",
        )
        .unwrap();
        let body = fs::read_to_string(&note).unwrap();
        assert!(body.contains("http://127.0.0.1:7700/v1"));
        assert!(body.contains("joule_deadbeefcafebabe"));
        assert!(body.contains("kimi-open"));
